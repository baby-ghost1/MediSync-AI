    ...
    {Array.from({ length: rows }).map((_, i) => (
      <div key={i} className="flex gap-4">
        {Array.from({ length: cols }).map((_, j) => (
          <div key={j} className={cn(shimmer, "h-8 rounded-lg", j === 0 ? "w-1/3" : "flex-1")} />
        ))}
      </div>
    ))}
  </div>
...
export const ListSkeleton = ({ items = 4, className }) => (
  <div className={cn("space-y-3", className)}>
  ...
  <div className={cn("rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5", className)}>
    ...
    <div className="flex items-end gap-2.5" style={{ height: 180 }}>
      {[45, 72, 58, 85, 63, 90, 50, 78].map((h, i) => (
        <div key={i} className={cn(shimmer, "flex-1 rounded-t-lg")} style={{ height: `${h}%` }} />
      ))}
    </div>
  </div>
);
...
export const DashboardGridSkeleton = ({ cards = 4 }) => (
  <div className="space-y-6">
    <div className={cn(shimmer, "h-8 w-64")} />
    ...
    <div className="grid gap-5 xl:grid-cols-3">
      <div className={cn("rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 xl:col-span-2")}>
        <div className={cn(shimmer, "mb-5 h-4 w-32")} />
        <div className="flex items-end gap-2.5" style={{ height: 180 }}>
          {[45, 72, 58, 85, 63, 90, 50, 78].map((h, i) => (
            <div key={i} className={cn(shimmer, "flex-1 rounded-t-lg")} style={{ height: `${h}%` }} />
